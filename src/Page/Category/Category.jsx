...
const Category = () => {
    ...
    const handleCategoryClick = async (category) => {
        try {
            const response = await fetch(`https://job-finde-server-lake.vercel.app/jobs?category=${category}`);
            if (response.ok) {
    ...
    return (
        <div >
            <h1 className="text-center mt-6 text-xl text-red-500">FEATURED TOURS PACKAGES</h1>
            ...
            <Tabs>
                <div className="flex mx-20 lg:mx-0 justify-center gap-8 ">
                    <TabList>
                        <Tab onClick={() => handleCategoryClick('')}>
                            <div>
